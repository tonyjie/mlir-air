...
// (matrix_multiplication/bf16_in_fp32_out/mm_aie2p.cc) uses aie::mmul<8,8,8>
// and measures 9797 GFLOP/s over 32 cores = 98 MAC/cycle/core. Speculative
// decoding (DFlash) needs a batch of 16 tokens per call, and widening the GEMV
// loop would keep the slow form -- so this takes the prefill's matmul and feeds
// it unpacked q4k weights.
//
...
// (it already is, in the Makefile's PEANO_KBASE). Measured on
// q4k_mmul<32,256,16>: 1787 bundles with it, 11088 without -- the emulation is
// a 3.9x SPEEDUP on the multiply, not an overhead. Building without it drops
// the batch at which this kernel stops being memory bound from 16.7 to 2.7,
// which would remove the reason to batch at all.
//
...
// lengths does not work here: the trip count lives in a register, so both
// builds emit the same body.)
//
...
// the multiply is. Q4K_UNPACK_NOPERM (numerically wrong, see below) restores
// the old fully-unrolled unpack for comparison against the earlier numbers.
...
//
//     Wb[ (j*(KCOL/8) + i)*64 + s_in*8 + t_in ]   row = 8j + t_in, col = 8i +
//     s_in
//
// A chunk covers ONE contraction block and SIXTEEN rows, so it splits into the
// two output blocks j = 2R and j = 2R+1. Within the chunk, lane = j_col*16 +
// rr, so group-of-8 index lane/8 is even exactly when rr < 8: filter_even with
// a chunk size of 8 collects rows 0..7 of every contraction step, in
// contraction order, which IS the j=2R tile. filter_odd gives j=2R+1. Two
// 64-lane stores, no shuffle network.
...
static inline void q4k_mmul(const bf16 *__restrict pA,
                            const bf16 *__restrict pB, float *__restrict pC) {
...
//   if it is the schedule      -- the bodies are fully unrolled and adjacent,
//   so
...
static inline void q4k_mm_chunked(const q4k_block_t *A,
                                  const bf16 *__restrict B, float *__restrict C,
                                  bf16 *__restrict W) {
  Q4K_MM_LOOP
  for (int c = 0; c < NCHUNK; c++) {
    ...
    [[clang::always_inline]] q4k_mmul<MROWS, KCOL, BATCH>(B + c * KCOL * BATCH,
                                                          W, C);
  }
}
