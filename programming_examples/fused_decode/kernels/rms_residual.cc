...
void rms_scale_row_partial_aie(float *restrict scales, bf16 *restrict x, int t,
                               int row_stride, int off, int n, int first,
                               int _arm) {
  (void)_arm;
  constexpr int vector_size = 16;
...
void rms_scale_row_partial_banded_aie(float *restrict scales, bf16 *restrict x,
...
static inline void rms_chunk(bf16 *restrict y, bf16 *restrict x,
  ...
  for (int t = 0; t < batch; t++) {
    ...
    // loop that is always XCHUNK/16 = 32. Under Peano
    // AIE_PREPARE_FOR_PIPELINING expands to NOTHING -- it is Chess-only, see
    // aie_kernel_utils.h -- so the hints that do anything on this toolchain are
    // the unroll and the range.
    AIE_LOOP_RANGE(8)
    AIE_LOOP_UNROLL(8)
    for (int i = 0; i < n / vector_size; i++) {
      aie::vector<bf16, vector_size> x_vec = aie::load_v<vector_size>(it_x);
...
// swallow it.
  ...
  for (int t = 0; t < batch; t++) {
    const auto c =
        aie::broadcast<bf16, vector_size>((bf16)((float)(t + 1) * 0.125f));
    bf16 *it_y = y + t * n;
    for (int i = 0; i < n / vector_size; i++)
