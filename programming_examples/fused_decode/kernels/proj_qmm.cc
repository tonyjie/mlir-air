...
//
// The seed is volatile so the recurrence is not constant-folded, and the step
// is XORSHIFT rather than the obvious multiply-add. That is not a style choice.
// s = a*s + c is AFFINE, so N of them compose into one a^N*s + C, and Peano
// does exactly that: at -O2 it unrolled by 32 and emitted 8 multiply-adds
// carrying a^4 = 0x979e791, so a sweep of "N units" was really running N/4 of
// them. Xorshift has no closed form under composition, so N steps cost N steps.
...
void proj_qmm_pass256(bf16 *__restrict x_blk, bf16 *__restrict w,
  ...
  // vectorised kernel measures its own deoptimisation. See the
  // RMS_CHUNK_PROBE=2 comment in rms_residual.cc.
  constexpr int vs = 16;
  static_assert(Q4NX_ROW_BLOCK_SIZE % vs == 0,
                "row block must be whole vectors");
...
//   register-only, xorshift           0.0436 -> 0.1186      convex, still
//   rising
//
...
void proj_qmm_mm_acc(bf16 *__restrict x_tile, bf16 *__restrict w,
  ...
  for (int m = 0; m < PROJ_MM_BATCH * Q4NX_ROW_BLOCK_SIZE / vs; m++)
    aie::store_v(y_acc + m * vs, (aie::vector<float, vs>)aie::mul(
                                     aie::load_v<vs>(x_tile + m * vs), one));
...
void proj_qmm_mm_flush_row(float *__restrict y_acc, bf16 *__restrict y_out,
  ...
  //      not a token permutation. See l1_align.py and
