...
"""Attention Half of Transformer Block -- 9-launch multi-launch ELF.

Builds a single AIR function with 9 sequential air.launch operations
implementing the attention half of one LLaMA-3.2-1B transformer layer:
  ...
  L9:  Residual Add [8,1]   proj(collapse) + x_in(collapse) -> output(1D)

17 func args (9 launches). All intermediates are passed as func args so
that each launch can be independently tested. Collapse shapes are used
inside launches for 2D<->1D aliasing (RoPE, residual add).

Usage:
    python3 attn_half_multi.py -p           # print combined MLIR
    python3 attn_half_multi.py              # compile + run + validate
"""
...
def _build_add_2d_to_1d(rows, cols, np_dtype, vector_size, herd_x, herd_y):
    ...
    memref. This is the residual add producing the flat output.
    """
...
def build_attn_half_module(
    ...
    """Build 9-launch attention half module.

    Combines: RMSNorm + QKV GEMMs + RoPE Q/K + FlashAttention + O GEMM +
    Residual Add

    Returns:
        Module with func @attn_half and 17 memref args:
            ...
            %arg16: output       memref<n_total x bf16>              1D final output
    """
    ...
    print(
        f"  [5/9] RoPE Q (outer={seq_len}x{emb_dim}, "
    ...
    print(
        f"  [6/9] RoPE K (outer={seq_len}x{kv_dim}, "
        f"embed_dim={head_dim}, herd_x={rope_herd_x})..."
    )
    ...
    print(
        f"  [7/9] FlashAttention (lq={seq_len}, lk={seq_len}, "
    ...
    # L9: Residual Add (2D + 2D -> 1D, collapse inside launch)
    print("  [9/9] Residual Add (2D -> 1D)...")
    add_ir = str(
    ...
    all_privates = set()
    for ir in [rms_ir, rope_q_ir, attn_ir]:
    ...
    # Assemble the combined module (17 func args, 9 launches)
    combined = "\n".join(maps_all) + f"""
module {{
  {privates_str}
  {channels_str}
...
"""
    ...
    with Context() as ctx:
        try:
            ...
            with open("/tmp/debug_attn_half.mlir", "w") as f:
                f.write(combined)
            print(f"  PARSE ERROR: Combined MLIR dumped to /tmp/debug_attn_half.mlir")
            raise
...
def attn_half_reference(
    ...
    """Attention half CPU reference (F32 computation, BF16 output).
    ...
    Returns:
        output: (seq_len, emb_dim) final output (proj + x_in)
    """
    ...
    # 9. Residual add
    output = (proj.astype(np.float32) + x_in.astype(np.float32)).astype(bfloat16)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Attention Half -- 9-launch multi-launch test"
    )
    ...
    print(
        f"Attention Half Multi-Launch: seq={SEQ_LEN}, emb={EMB_DIM}, "
        f"kv={KV_DIM}, heads={N_HEADS}/{N_KV_HEADS}, dk={HEAD_DIM}"
    )

    module = build_attn_half_module(
        seq_len=SEQ_LEN,
        emb_dim=EMB_DIM,
        kv_dim=KV_DIM,
        n_heads=N_HEADS,
    ...
    output_ref = attn_half_reference(
        x_in,
        attn_norm_w,
        wq,
        wk,
    ...
    # Func signature: 17 args
    # inputs = args 0-15 (including zeroed intermediates)
    # expected_outputs = arg 16 (1D output)
    runner = XRTRunner(
        verbose=args.verbose,
        omit_while_true_loop=False,
        output_format=args.output_format,
    ...
    exit(
        runner.run_test(
            ...
            expected_outputs=[
                output_ref.reshape(-1),  # arg16: output (1D)
            ],
            rtol=0.2,
            atol=4.0,
