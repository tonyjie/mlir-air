...
External `.o` files needed: `mv.o` (GEMV kernel), `rope.o` (RoPE kernel), and `silu_and_mul.o` (SiLU kernel) must be in the working directory. The script copies them from their respective `build_peano/` directories.
...
```
llama3_decode.py
  ├── compile_decode_kernels()       compile 9 kernels to decode_kernel_cache/
  ├── decode_attention_cpu()          single-query attention with KV cache
  ├── run_decode_block()             1 transformer block (10 NPU calls + CPU attn)
...
| `o_gemv_add` | Multi-launch (2 launches) | `o_gemv_add_multi.py` | O: 2048×2048, Add: n=2048 | [8,1] + [8,1] | ELF |
...
| `add` | Single | `eltwise_add.py` | n=2048 | [8,1] | xclbin |
| `silu_mul` | Single | `silu_and_mul.py` | n=8192 | [8,1] | xclbin |
| `rope_q` | Single | `rope_lut.py` | 32×64 | [1,1] | xclbin |
| `rope_k` | Single | `rope_lut.py` | 8×64 | [1,1] | xclbin |

**Multi-launch ELFs**: Multiple `air.launch` ops stitched into one MLIR module using text-based MLIR stitching (same technique as prefill). One `xrt.run()` executes all launches sequentially. This reduces XRT dispatch overhead from 15 to 10 calls per block.
...
`run_decode_block()` runs 10 NPU calls + CPU attention per block:
...
NPU call 5: o_gemv_add (2-launch ELF)         [8,1] + [8,1] herd, ELF
...
NPU call 8: silu_mul                          [8,1] herd, xclbin
  swiglu = SiLU(gate) × up (8192 elements)

NPU call 9: gemv_down                         [8,1] herd, xclbin
  swiglu × Wdown.T → down (2048,)

NPU call 10: add                              [8,1] herd, xclbin
...
For each of 16 transformer layers:
  ...
  20. NPU call 5: o_gemv_add                [8,1]+[8,1], ELF
  ...
  23. NPU call 8: silu_mul                  [8,1] herd, xclbin
      write: gate(16KB) + up(16KB) + output(16KB)
      → SiLU(gate) × up
      Total per-call: ~0.3ms (steady state)

  24. NPU call 9: gemv_down                 [8,1] herd, xclbin
      TOKEN 0: write w_down(32MB) + swiglu(16KB)
      TOKEN 1+: skip w_down (static), write swiglu(16KB) only
      Total per-call: ~2.1ms (steady state)

  25. NPU call 10: add                      [8,1] herd, xclbin
      write: res1(4KB) + down(4KB) + output(4KB)
      Total per-call: ~0.3ms

End of 16 layers
  Total per-token for 16 layers: ~126ms NPU + ~43ms CPU
...
| Total time | ~893ms | ~351ms |

The ~542ms difference is entirely from weight BO writes on the first token.
...
- All 10 NPU kernel invocations per block × 16 blocks = 160 NPU calls
...
| **Time/token (avg, all 100)** | **356ms** | **370ms** | **AIR 4% faster** |
...
| SiLU×mul | 8 compute tiles | `herd=[8,1]`: each tile processes 1024 elements |
| RMSNorm | 1 compute tile | `herd=[1,1]`: single tile processes all 2048 elements |
| Eltwise Add | 8 compute tiles | `herd=[8,1]`: each tile processes 256 elements |
...
### 6.1 Multi-Launch ELF Fusion (15 → 10 calls per block)
...
**Note**: A 5-launch FFN merge (Gate+Up+SiLU+Down+Add) was attempted but blocked by a memref type mismatch: `linalg_fill_bf16` has signature `(bf16, memref<8xbf16, 2>)` for gate/up GEMV (tile_m=8) but needs `(bf16, memref<2xbf16, 2>)` for down GEMV (tile_m=2). These can't share the same private func declaration in one module.
...
| Aspect | AIR | IRON |
|---|---|---|
| **FFN** | 3 GEMV calls + NPU SiLU×mul + NPU Add | 1 fused SwiGLU (5 runlist entries) |
...
| **Dispatches per block** | 10 | ~12 |
| **NPU kernel time** | ~126ms | 132ms |
| **Total per token** | ~351ms | ~370ms |

The NPU kernel compute is slightly faster in AIR (~126ms vs 132ms) because our decomposed GEMVs avoid IRON's fused SwiGLU latency (4.8ms per block × 16 = 76ms for SwiGLU alone). Our gate+up+silu+down GEMVs total ~82ms for the same computation but with more dispatch overhead.
