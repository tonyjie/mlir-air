...
|-------|------------|------|---------|
| **Prefill** (seq_len=2048) | **1.30s kernel / 1.54s wall** | 2.744s | **2.1x (kernel)** |
| **Decode** (steady-state) | **92ms/token (10.8 tok/s)** | 370ms/token | **4.0x** |

**Unified pipeline**: `make compile && make run` (see `docs/usage.md`)

### Prefill (3 NPU invocations/layer)
- `rms_gemms_rope` (6 launches): RMSNorm + Q/K/V GEMM + RoPE Q+K
- `flash_attn` (1 launch): FlashAttention GQA, seq-first layout
- `o_ffn` (8 launches): O GEMM + Add + RMSNorm + FFN + Add
- Final: rmsnorm + lm_head (8-partition GEMM)

### Decode (3 NPU invocations/token)
- `rms_gemv_rope` (6 launches): RMSNorm + Q/K/V GEMV + RoPE Q+K
- CPU attention (GQA with KV cache)
- `o_gemv_ffn` (8 launches): O GEMV + Add + RMSNorm + FFN GEMV + Add
- Final: CPU rmsnorm + `lm_head_gemv` (8-partition GEMV, NPU)

### Key optimizations applied
- Multi-launch merging: 10→3 prefill calls/layer, 10→3 decode calls/block
- Per-layer weight BO pre-loading with `static_input_indices`
- `intermediate_indices`: skip host↔device sync for kernel-overwritten buffers
- NPU LM Head GEMV for decode (CPU 258ms → NPU 13.5ms)
- External kernel rename (`-D` preprocessor) for K=2048/K=8192 GEMV coexistence
- Seq-first layout: zero host transposes between prefill kernels
...
make profile  # run with profiling breakdown
```

See `docs/usage.md` for full command reference, `docs/profile.md` for performance breakdown.
...
## Phase 5: Decode Optimization (April 2026)

| Date | Milestone |
|------|-----------|
| 2026-04-08 | Multi-launch decode merging: 10→2 XRT calls/block (rms_gemv_rope + o_gemv_ffn) |
| 2026-04-08 | External kernel rename (`-D` preprocessor): solved K=2048/K=8192 GEMV type mismatch |
| 2026-04-08 | `intermediate_indices` optimization: skip unnecessary BO syncs |
| 2026-04-09 | NPU LM Head GEMV: 258ms CPU → 13.5ms NPU (18.6× speedup) |
| 2026-04-09 | Decode: **92ms/token (10.8 tok/s)** — 4.0× faster than IRON |

---

## Phase 6: Code Reorganization (April 2026)

| Date | Milestone |
|------|-----------|
| 2026-04-09 | Created `kernel_builder/` package (stitching, gemm_builder, cache, external_kernels) |
| 2026-04-09 | All external C++ kernels compiled from source (no stale .o copies) |
| 2026-04-09 | Per-layer weight BO pre-loading: prefill BO overhead 14% → 4% |
| 2026-04-09 | Unified `llama3_inference.py`: NPU prefill + NPU decode in one script |
| 2026-04-10 | Makefile: `make compile`, `make run`, `make profile`, `make verify`, `make clean` |
| 2026-04-10 | Docs reorganization: usage.md, profile.md, explain.md, issues.md |
...
make run         # run inference (100 tokens)
make profile     # run with profiling breakdown
make verify      # verify against CPU reference
