...
"""CPU reference implementation of SmolLM2-1.7B forward pass.
...
SmolLM2-1.7B config:
  24 layers, emb_dim=2048, n_heads=32, head_dim=64,
  n_kv_heads=32 (MHA — degenerate GQA with group_size=1),
  hidden_dim=8192, vocab_size=49152, BF16, rope_base=130000,
  tied embeddings (lm_head shares embed_tokens.weight).

The kernel-side `attention_reference` already supports MHA as a degenerate
case of GQA (group_size = n_heads // n_kv_heads = 1) — no algorithmic change
needed vs. the Llama-3.2-1B reference.
"""

import argparse
import numpy as np

from smollm2_weights import (
    LlamaConfig,
    LayerWeights,
    LlamaWeights,
    load_weights,
...
def attention_reference(q, k, v, n_heads, n_kv_heads):
    ...
    For SmolLM2-1.7B (MHA) the degenerate case n_kv_heads == n_heads is used,
    giving group_size = 1 — i.e. each query head has its own KV head.
    ...
    Args:
        ...
        n_kv_heads: Number of key/value heads (== n_heads for MHA).
    ...
    for h in range(n_heads):
        kv_idx = h // group_size  # for MHA: kv_idx == h
...
def transformer_block(x, layer_weights, rope_lut, config):
    ...
    k = normed @ wk  # (seq_len, n_kv_heads * head_dim) = (seq_len, 2048) for MHA
    v = normed @ wv  # (seq_len, n_kv_heads * head_dim) = (seq_len, 2048) for MHA
    intermediates["q"] = q
    intermediates["k"] = k
    intermediates["v"] = v
...
def forward(token_ids, weights, config, rope_lut=None):
    """Full SmolLM2-1.7B forward pass."""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="CPU reference forward pass for SmolLM2-1.7B"
    )
    parser.add_argument(
        "--model",
        type=str,
        default="HuggingFaceTB/SmolLM2-1.7B",
        help="HuggingFace model name or local path (default: HuggingFaceTB/SmolLM2-1.7B)",
    )
    parser.add_argument(
        "--prompt",
        type=str,
