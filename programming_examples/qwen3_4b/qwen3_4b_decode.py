...
"""Step 5 — Qwen3-4B NPU decode block + generate loop.
...
from qwen3_4b_weights import LlamaConfig, generate_rope_lut
...
def compile_decode_kernels(cache, config):
    ...
    # Qwen3-4B per-launch tile_m (Rule D L2 cap; tile_m must be % m_input == 0):
    #   O GEMV   K=q_dim=4096  → tile_m=4, m_input=4 (A=256KB ≤ 512KB)
    #   Gate/Up  K=emb=3072    → tile_m=8, m_input=4 (A=384KB ≤ 512KB; default mv.o)
    #   Down     K=hidden=10240 → tile_m=2, m_input=2 (A=320KB ≤ 512KB; tile_m%m_input==0)
    _O_TILE_M = 4
    _O_M_INPUT = 4
    _DOWN_TILE_M = 2
    _DOWN_M_INPUT = 2
    compile_mv_og(tile_m=_O_TILE_M)
    compile_mv_dg_qwen3(tile_m=_DOWN_TILE_M)
    ...
    if "o_gemv_ffn_silu" not in cache.artifacts:
        print(
            f"  [o_gemv_ffn_silu] building (8-launch fused, 3-K rename, "
            f"o_tile_m={_O_TILE_M}, down_tile_m={_DOWN_TILE_M})..."
        )
        cache.compile_and_cache(
            "o_gemv_ffn_silu",
            build_o_gemv_ffn_silu_qwen3_module(
                ...
                o_in_dim=q_dim,
                o_tile_m=_O_TILE_M,
                o_m_input=_O_M_INPUT,
                down_tile_m=_DOWN_TILE_M,
                down_m_input=_DOWN_M_INPUT,
            ),
            _O_GEMV_FFN_SILU_BACKEND,
        )
    else:
    ...
    if "lm_head_gemv" not in cache.artifacts:
        # Qwen3-4B at K=padded_emb=3072: 10×16384 fails Rule C (per-partition
        # launches = 16384/(8*8) = 256 > 255). Use 19×8192 (matches qwen3_1_7b
        # precedent at vocab=151936); per-partition launches = 8192/64 = 128 ≤ 255 ✓
        # A buffer = 3072 × 8 × 8 × 2 = 384 KB ≤ 512 KB ✓ (Rule D).
...
_LM_PARTITIONS = 19
_LM_PART_SIZE = 8192  # 19 × 8192 = 155648 ≥ vocab=151936 (3712 row pad)
...
def preload_decode_weights(cache, weights, config):
    ...
    """
    if getattr(weights, "_qwen3_4b_decode_preloaded", False):
        return
    n_layers = config.n_layers
    n_heads = config.n_heads
    n_kv_heads = config.n_kv_heads
    ...
    weights._qwen3_4b_decode_preloaded = True
    print(
