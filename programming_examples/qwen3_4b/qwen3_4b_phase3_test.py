...
"""Phase 3 — full 36-layer correctness for Qwen3-4B on NPU2.

Wires all 36 transformer blocks via the split-ELF helper from Phase 2,
...
Uses PADDED config (emb 2560→3072, hidden 9728→10240) — required for the
fused rms_attn_gemms ELF; see phase2_block.md for the W1 debug story.
...
Per-layer cosine drift is informational at n_layers=36, head_dim=128
...
from qwen3_4b_phase2_test import _compile_qwen3_block_kernels, run_qwen3_block_npu
...
def main():
    parser = argparse.ArgumentParser(
        description="Qwen3-4B Phase 3 full 28-layer correctness test"
    )
    parser.add_argument(
        "--seq-len",
        type=int,
    ...
    print(
        f"Qwen3-4B Phase 3 — full {config.n_layers}-layer NPU forward "
        f"vs CPU reference (seq_len={args.seq_len}, "
        f"PADDED emb={config.emb_dim}, hidden={config.hidden_dim})"
    )
