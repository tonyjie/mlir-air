...
def main():
    parser = argparse.ArgumentParser(
        description="Qwen3-4B Phase 5 decode (NPU prefill seed + CPU decode)"
    )
    parser.add_argument(
        "--seq-len",
        type=int,
    ...
    print(
        f"Qwen3-4B Phase 5 — decode (PADDED emb={config.emb_dim}, hidden={config.hidden_dim}); "
        f"NPU prefill at seq_len={args.seq_len} + NPU decode {args.n_tokens} tokens"
    )
