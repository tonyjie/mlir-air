...
"""Phase 4 — prefill perf for Qwen3-4B on NPU2.
...
def main():
    parser = argparse.ArgumentParser(
        description="Qwen3-4B Phase 4 prefill perf measurement"
    )
    parser.add_argument(
        "--seq-len",
        type=int,
    ...
    print(
        f"Qwen3-4B Phase 4 — prefill perf (seq_len={args.seq_len}, "
        f"{config.n_layers} layers, PADDED emb={config.emb_dim}, hidden={config.hidden_dim})"
    )
