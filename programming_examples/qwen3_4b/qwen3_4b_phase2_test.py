...
"""Phase 2 — single-block correctness for Qwen3-4B on NPU2.

Wires layer 0 with the split-ELF approach (mirror of qwen3_1_7b/0_6b):
  ...
  5. NPU o_ffn (with o_in_dim=q_dim=4096; emb_dim=2560; hidden_dim=9728)

Qwen3-4B specifics:
  - emb_dim=2560 NOT 1024-aligned (Rule A risk; first attempt unpadded)
  - hidden_dim=9728 NOT 1024-aligned (Rule A risk)
  - q_dim=4096 ≠ emb_dim=2560
  - 32 Q heads × 8 KV heads (GQA group=4)
...
import qwen3_4b_reference as qwen3_reference
from qwen3_4b_pad import make_padded_config, pad_weights, pad_input_embed, slice_output
...
def _compile_qwen3_block_kernels(cache, config, seq_len):
    ...
    if "rms_attn_gemms" not in cache.artifacts:
        ...
        module = build_rms_attn_gemms_module(
            seq_len=seq_len,
            emb_dim=config.emb_dim,  # PADDED 3072 (1024-aligned)
            kv_dim=kv_dim,
            q_dim=q_dim,
            tile_n=128,
            herd_n=4,
...
def main():
    parser = argparse.ArgumentParser(
        description="Qwen3-4B Phase 2 single-block test (NPU)"
    )
    parser.add_argument(
        "--seq-len",
        type=int,
    ...
    # Pad emb 2560→3072, hidden 9728→10240 (Rule A workaround for fused
    # rms_attn_gemms ELF; first attempt unpadded gave Q/K/V cosine=0
    # garbage at K=2560).
    config = make_padded_config(
        orig_config, padded_emb_dim=3072, padded_hidden_dim=10240
    )
    print(
        f"Qwen3-4B PADDED config: n_layers={config.n_layers}, "
        f"emb_dim={orig_config.emb_dim}→{config.emb_dim}, n_heads={config.n_heads}, "
        f"n_kv_heads={config.n_kv_heads} (GQA group={config.n_heads // config.n_kv_heads}), "
        f"head_dim={config.head_dim}, hidden_dim={orig_config.hidden_dim}→{config.hidden_dim}, "
    ...
    print("Padding weights (emb 2560→3072, hidden 9728→10240, RMSNorm rescale)...")
    t = time.time()
    weights = pad_weights(orig_weights, orig_config, config)
    print(f"  Padded in {time.time()-t:.1f}s")
    ...
    # Use PADDED embed_table directly so x is naturally padded to padded_emb.
    ...
    print("\nRunning CPU reference single block (layer 0, padded)...")
    t = time.time()
