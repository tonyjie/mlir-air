...
"""CPU reference forward pass for Qwen3-4B (Qwen3ForCausalLM).

Pure NumPy in F32 for high-accuracy verification of NPU outputs.
Mirrors qwen3_1_7b/qwen3_reference.py — Qwen3 family conventions are
identical (NO QKV bias, Q/K Norm before RoPE, half-split RoPE).
Only the config (36L / emb=2560 / 32H / 8KV / hidden=9728) differs.
"""
...
def attention_reference(q, k, v, n_heads, n_kv_heads):
    ...
    For Qwen3-4B: 32 Q heads, 8 KV heads, group_size = 4.
    """
...
def forward(token_ids, weights, config, rope_lut=None, capture_layer_out=False):
    """Full Qwen3-4B forward pass."""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="CPU reference forward pass for Qwen3-4B"
    )
    ...
    parser.add_argument(
        "--verify",
        action="store_true",
        help="Compare against HuggingFace transformers reference (per-layer + final logits + top-1)",
    )
    ...
    if args.verify:
        logits, per_layer_out = forward(
            token_ids, weights, config, capture_layer_out=True
        )
    else:
    ...
    if args.verify:
        ...
        try:
            ...
            print("Loading HuggingFace model (F32)...")
            ...
            with torch.no_grad():
                ...
                # output_hidden_states: tuple length n_layers+1; index i+1 is layer i output
                hf_hidden = [h[0].numpy() for h in hf_output.hidden_states]

            print(f"HF logits shape: {hf_logits.shape}")

            # Per-layer cosine. NOTE: in modern transformers, hf_hidden has
            # length n_layers+1; index 0 = embeddings, index i+1 (i in
            # 0..n_layers-2) = layer i output, index n_layers = post-final-norm
            # output. So compare per_layer_out[i] vs hf_hidden[i+1] for the
            # first n_layers-1 layers; for the last layer, compare
            # final_norm(per_layer_out[-1]) vs hf_hidden[n_layers].
            print(f"\nPer-layer cosine vs HF (gate ≥ 0.99):")
            min_layer_cos = 1.0
            min_layer_idx = -1
            fail_layers = []
            final_norm_w = np.asarray(weights.final_norm, dtype=np.float32)
            for i in range(config.n_layers):
                if i == config.n_layers - 1:
                    ours = rms_norm(
                        per_layer_out[i], final_norm_w, eps=config.rms_norm_eps
                    )
                    label = f"Layer {i:2d} (post-final-norm)"
                else:
                    ours = per_layer_out[i]
                    label = f"Layer {i:2d}"
                cos = _cosine(ours, hf_hidden[i + 1])
                marker = "✓" if cos >= 0.99 else "✗"
                if cos < min_layer_cos:
                    min_layer_cos = cos
                    min_layer_idx = i
                if cos < 0.99:
                    fail_layers.append((i, cos))
                if i < 4 or i >= config.n_layers - 4 or cos < 0.99:
                    print(f"  {label}: {cos:.6f}  {marker}")
            print(
                f"  ... min across {config.n_layers} layers: "
                f"L{min_layer_idx}={min_layer_cos:.6f}"
            )
            ...
            print(
                f"\nFinal logits cosine at pos {pred_pos}: {final_cos:.8f} "
                f"(gate ≥ 0.999)"
            )
            ...
            top1_match = our_top1 == hf_top1
            print(f"\nTop-1 prediction match: {'YES' if top1_match else 'NO'}")
            print(f"  Ours: '{tokenizer.decode([our_top1])}' (id={our_top1})")
            print(f"  HF:   '{tokenizer.decode([hf_top1])}' (id={hf_top1})")

            # Pass criteria.
            per_layer_pass = len(fail_layers) == 0
            final_pass = final_cos >= 0.999
            print()
            print("=== Phase 0 PASS criteria ===")
            print(
                f"  per-layer cos ≥ 0.99 (all):    "
                f"{'PASS' if per_layer_pass else f'FAIL ({len(fail_layers)} layers)'}"
            )
            print(
                f"  final logits cos ≥ 0.999:      {'PASS' if final_pass else 'FAIL'}"
            )
            print(
                f"  top-1 strict match vs HF:       {'PASS' if top1_match else 'FAIL'}"
            )

            if per_layer_pass and final_pass and top1_match:
                print("\nVERIFICATION PASSED")
                sys.exit(0)
            else:
                print("\nVERIFICATION FAILED")
