...
def build_o_gemv_ffn_silu_qwen3_module(
    ...
    m_input=4,
    herd_m=8,
    # Per-launch tile_m overrides (Qwen3-4B needs these because q_dim=4096
    # and hidden_padded=10240 exceed Rule D L2 cap at uniform tile_m=8):
    o_tile_m=None,
    o_m_input=None,
    down_tile_m=None,
    down_m_input=None,
    ...
    Per-launch tile_m / m_input: pass o_tile_m / down_tile_m to override the
    uniform `tile_m` for the O / Down GEMV launches (the corresponding
    mv_og.o / mv_dg_qwen3.o must be compiled with matching DIM_M_OUTPUT).
    Pair with o_m_input / down_m_input to satisfy `tile_m % m_input == 0`.
    """
    if o_tile_m is None:
        o_tile_m = tile_m
    if o_m_input is None:
        o_m_input = m_input
    if down_tile_m is None:
        down_tile_m = tile_m
    if down_m_input is None:
        down_m_input = m_input
    ...
    print(
        f"  [1/8] O GEMV (m={emb_dim}, k={o_in_dim}, tile_m={o_tile_m}, m_input={o_m_input})..."
    )
    o_ir = str(
        build_gemv(emb_dim, o_in_dim, o_tile_m, o_m_input, herd_m, bfloat16, bfloat16)
    )
    ...
    print(
        f"  [7/8] Down GEMV (m={emb_dim}, k={hidden_dim}, tile_m={down_tile_m}, m_input={down_m_input})..."
    )
    down_ir = str(
        build_gemv(
