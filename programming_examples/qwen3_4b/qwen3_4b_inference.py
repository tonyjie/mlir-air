...
"""End-to-end Qwen3-4B inference on NPU2.
...
Usage:
    ...
    python3 qwen3_4b_inference.py --n-tokens 30 --prompt "The capital of France is"
    python3 qwen3_4b_inference.py --decode cpu          # legacy slow path
    python3 qwen3_4b_inference.py --profile
"""
...
from qwen3_4b_phase2_test import _compile_qwen3_block_kernels
from qwen3_4b_phase4_test import npu_full_prefill
from qwen3_4b_phase5_test import cpu_decode_token
import qwen3_4b_decode as qwen3_decode


def main():
    parser = argparse.ArgumentParser(
        description="End-to-end Qwen3-4B NPU prefill + CPU decode"
    )
    ...
    parser.add_argument(
        "--cache-dir", type=str, default="build/prefill_kernel_cache_2048"
    )
    parser.add_argument(
        "--decode-cache-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "cosine + max_err + mean_err for all 36 layers, plus final logits "
            "cosine at pred_pos and top-1 match. Mirrors llama3 / llama32_3b "
            "/ smollm2 / qwen25 / qwen3-0.6B --verify. Adds ~30s wall."
        ),
    )
    ...
    print(
        f"Qwen3-4B end-to-end NPU inference (PADDED emb={config.emb_dim}, "
        f"hidden={config.hidden_dim}; seq_len={args.seq_len}, {args.n_tokens} decode tokens)"
    )
    ...
    if args.verify:
        ...
        print(f"{'='*60}")
        from qwen3_4b_reference import (
