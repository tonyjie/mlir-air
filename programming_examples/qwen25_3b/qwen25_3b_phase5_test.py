...
"""Phase 5 — decode performance for Qwen2.5-3B on NPU2.

Mirrors qwen25_0_5b/qwen25_0_5b_phase5_test.py with 3B shape:
- emb=2048, hidden=11008, n_h=16, n_kv=2, hd=128 (orig shapes for decode)
- mv_k11008.o (DIM_M_OUTPUT=2 + down_k_split=86)
- 10×16384 LM head GEMV partitions (vocab=151936 same as 1.5B/0.5B)

Pattern application (5/5):
  A.  Multi-launch merging      INHERITED (rms_gemv_rope=6, o_gemv_ffn=8)
  B.  Static weight BOs         INHERITED (preload_decode_weights)
  D1. NPU LM Head GEMV          APPLIED (10×16384 partitions)
  D2. Extern kernel rename      APPLIED (mv_k11008.o for Down GEMV K=11008)
...
def main():
    parser = argparse.ArgumentParser(description="Qwen2.5-3B Phase 5 decode perf")
    ...
    config = LlamaConfig()
    print(
        f"Qwen2.5-3B (orig shapes for decode): n_layers={config.n_layers}, "
    ...
    if args.compile_only:
        ...
        if (_THIS_DIR / args.cache_dir / "manifest.json").exists():
            cache.load_manifest()
        compile_qwen25_3b_decode_kernels(cache, config)
    ...
    if not all(k in cache.artifacts for k in needed):
        print("\nCompiling decode kernels for Qwen2.5-3B shapes...")
        t = time.time()
        compile_qwen25_3b_decode_kernels(cache, config)
    ...
    t = time.time()
    preload_qwen25_3b_lm_head(cache, weights, config)
    ...
    for token_idx in range(args.n_tokens - 1):
        ...
        x_normed_bf16 = (
            qwen25_3b_reference.rms_norm(x_f32, weights.final_norm)
            .flatten()
            .astype(bfloat16)
        )
        npu_logits = qwen25_3b_npu_lm_head_gemv(cache, weights, config, x_normed_bf16)
        ...
        if args.cpu_verify:
            ...
            for layer_idx in range(config.n_layers):
                cpu_x, _ = qwen25_3b_reference.transformer_block(
                    cpu_x, weights.layers[layer_idx], rope_lut_f32, config
                )
            cpu_normed = qwen25_3b_reference.rms_norm(cpu_x, weights.final_norm)
    ...
    print(
        "    [APPLIED  ] D2. Extern kernel rename     (mv_k11008.o for Down GEMV K=11008)"
    )
    print(
