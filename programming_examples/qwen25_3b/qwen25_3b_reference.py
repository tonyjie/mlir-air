...
"""CPU reference forward pass for Qwen2.5-3B (Qwen2ForCausalLM).

Pure NumPy in F32. Mirrors qwen25_0_5b/qwen25_0_5b_reference.py with
shape adapted to 3B (36 layers, emb=2048, n_h=16, n_kv=2, hd=128,
hidden=11008).

GQA reduction: n_heads=16, n_kv_heads=2 → group_size=8.
"""
...
def attention_reference(q, k, v, n_heads, n_kv_heads):
    """GQA: n_h=16, n_kv=2, group_size=8."""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="CPU reference forward pass for Qwen2.5-3B"
    )
    ...
    if len(token_ids) > args.seq_len:
        ...
        print(f"Padded from {original_len} to {args.seq_len} tokens")
    ...
    if args.verify:
        ...
        try:
            ...
            print(f"\nPer-layer hidden-state cosine vs HF (threshold ≥ 0.99):")
            per_layer_min = 1.0
            per_layer_max = 0.0
            per_layer_fail = 0
