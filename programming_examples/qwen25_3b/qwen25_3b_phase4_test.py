...
"""Phase 4 — prefill performance for Qwen2.5-3B on NPU2.

NPU forward uses padded shapes (emb=2048, hidden=12288, n_heads=16) plus
GQA-reindexed weights, Qwen2 host bias add, and NPU FA Option C
head-first wrapper (head_dim=128).
...
def main():
    parser = argparse.ArgumentParser(description="Qwen2.5-3B Phase 4 prefill perf")
    ...
    print(
        f"Qwen2.5-3B padded config: n_layers={padded_config.n_layers}, "
    ...
    print(f"Attention: {'CPU' if args.cpu_attn else 'NPU FA Option C (hd=128)'}")
    ...
    print(
        "  [INHERITED] C. Seq-first layout          (RoPE/FA native; no host transposes — host transposes only inside Option C wrapper)"
    )
    if args.cpu_attn:
        print("  [SKIPPED  ] D. CPU→NPU op promotion     (--cpu-attn explicitly set)")
    else:
        print(
            "  [APPLIED  ] D. CPU→NPU op promotion     (NPU FA Option C head-first; hd=128)"
        )
