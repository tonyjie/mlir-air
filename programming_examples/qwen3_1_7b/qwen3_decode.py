...
"""Step 5 — Qwen3-1.7B NPU decode block + generate loop.
...
# Qwen3-1.7B has q_dim==emb_dim==2048 → only 2 K values in o+ffn (K=2048
# Q/K/V/O/Gate/Up, K=6144 Down). Use llama3's standard 2-K-rename builder
# directly; the qwen3-0.6B-specific 3-K builder isn't needed here.
...
# llama3's o_gemv_ffn ELF needs DIFFERENT backend kwargs than the bare
# matvec ones in _GEMV_BACKEND_BASE (matches llama3_decode.py:_OGEMV_FFN_BACKEND).
...
def compile_decode_kernels(cache, config):
    ...
    # 1.7B uses llama3's standard 2-K rename — no compile_mv_og/compile_mv_dg_qwen3
    # needed (those are 0.6B-specific for the 3-K case).
    ...
    # 1.7B has q_dim==emb_dim → only 2 K values in o+ffn (K=2048, K=6144)
    # → existing llama3 2-K rename pattern works (mv.o + mv_k8192.o for Down).
    # No need to compile mv_og.o or mv_dg_qwen3.o for 1.7B.
    ...
    if "o_gemv_ffn_silu" not in cache.artifacts:
        print("  [o_gemv_ffn_silu] building (8-launch fused, llama3 2-K rename)...")
        cache.compile_and_cache(
            "o_gemv_ffn_silu",
            # Down at K=hidden_dim=6144 needs tile_m=2 (L2 fit) — mirrors
            # llama3's K=8192 path which uses the existing mv_k8192.o
            # (DIM_M_OUTPUT=2 + dg_matvec_* rename).
    ...
    if "lm_head_gemv" not in cache.artifacts:
        # At emb_dim=2048, K=2048 forces tighter L2: tile_m=16/herd_m=8
        # → A=524288B which exceeds 512KB - C=256B. Use 19 × 8192 instead
        # of 10 × 16384: A=262144B at tile_m=8/herd_m=8 ✓; per-partition
        # B-DMA fires = 8192/(8*8) = 128 ≤ 255 ✓; pad = 19*8192 - 151936
        # = 3712 rows.
        print(f"  [lm_head_gemv] building (19 × 8192, K={emb_dim})...")
        cache.compile_and_cache(
            "lm_head_gemv",
            build_lm_head_gemv_module(
                emb_dim=emb_dim,
...
def run_decode_block(
    ...
    # ---- 3. NPU o_gemv_ffn (8-launch fused) ----
    # O + add + RMSNorm + Gate + Up + SiLU+Mul + Down + add → block_out.
    # llama3 builder arg layout: wo(0), attn_out(1), proj(2), x_res(3),
    # res1(4), ffn_norm_w(5), normed2(6), w_gate(7), gate(8), w_up(9),
    # up(10), swiglu(11), w_down(12), down(13), out(14).
    offn_key = f"o_gemv_ffn_silu_L{layer_idx}"
    if offn_key not in _arg_cache:
        _ensure_decode_weights_transposed(layer_weights, config)
        _arg_cache[offn_key] = [
            layer_weights._wo_t,  # 0: wo (q_dim → emb_dim) — STATIC
            None,  # 1: attn_out (dynamic)
    ...
    offn_args[1] = np.asarray(attn_out, dtype=bfloat16).reshape(q_dim)
    ...
    res = _lp._run_cached(
        ...
        *offn_args,
        output_indices=[14],
        static_input_indices={0, 5, 7, 9, 12},  # weights — wo is now arg0
...
def preload_decode_weights(cache, weights, config):
    ...
    for li in range(n_layers):
        ...
        # llama3 o_gemv_ffn arg order: wo at arg0, attn_out at arg1.
        _lp._run_cached(
            cache,
            "o_gemv_ffn_silu",
            _O_GEMV_FFN_SILU_BACKEND,
            lw._wo_t,  # 0 wo
            np.zeros(q_dim, dtype=bfloat16),  # 1 attn_out
