...
def main():
    EMB_DIM = 2048  # Qwen3-1.7B (was 1024 in 0.6B)
    HIDDEN_DIM = 6144  # Qwen3-1.7B (was 3072 in 0.6B)
    O_IN_DIM = 2048  # n_heads * head_dim == emb_dim for 1.7B (no 3-K rename collision)
    ...
    module = build_o_gemv_ffn_module(
        ...
        m_input=4,
        herd_m=8,
        # Down at K=6144 must shrink tile_m to fit L2 (8×8×6144×2 = 768KB > 512KB).
        # tile_m=2 gives A=192KB, matches the existing mv_k8192.o (DIM_M_OUTPUT=2).
        down_tile_m=2,
        down_m_input=1,
    ...
    # llama3's o_gemv_ffn ELF needs DIFFERENT backend kwargs than the
    # bare-matvec ones used in qwen3-0.6B's o_gemv_ffn_silu_qwen3 (which
    # rebuilt the stitching from scratch). Match llama3_decode.py's
    # _OGEMV_FFN_BACKEND exactly to avoid all-zero outputs.
    backend = XRTBackend(
        verbose=False,
    ...
    # llama3 o_gemv_ffn arg order: wo (arg0), attn_out (arg1) — wo FIRST.
    # qwen3-0.6B's o_gemv_ffn_silu_qwen3 fork swapped these; we're using
    # llama3's builder directly so follow its order.
    inputs_all = [
        wo,
        attn_out_in,
        proj_buf,
