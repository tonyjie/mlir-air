...
//===----------------------------------------------------------------------===//
//
// Forked from programming_examples/softmax/softmax.cc.
//
// Adds a masked full-row softmax variant: masked_softmax_bf16(in, pos, mask,
// out) computes softmax(in + mask) instead of softmax(in). This is used to
// build non-causal (prefix/padding-mask) attention by composing:
//   S = Q @ K^T / sqrt(d)   (registry GEMM)
//   S += additive_mask      (0 = attend, finfo(bf16).min = masked out)
//   P = row-softmax(S)      (this kernel)
//   O = P @ V                (registry GEMM)
//
// The additive mask uses a large finite negative value (matching
// torch.finfo(bfloat16).min), NOT -inf, so that a fully-masked row (every
// entry masked) produces a uniform softmax row instead of NaN: when every
// entry in the row is the same very negative constant, the row max equals
// that same constant, so every (x - max) difference is exactly 0 and the
// row softmax is uniform.
//
//===----------------------------------------------------------------------===//
...
// Floor applied to (value - row_max) before it is quantized into the LUT's
// fixed-point index (see the comment at its use site in exp_bf16 below).
constexpr float kExpDiffFloor = -120.0f;
...
float exp_bf16(int num_elems, VecIteratorIn &in, VecIteratorOut &out) {
  ...
  for (int i = 0; i < elem_iters; i++)
    chess_prepare_for_pipelining chess_loop_range(4, ) {
      ...
      // Clamp (value - max) to a floor before quantizing to the LUT's
      // fixed-point index (SM_SCALE_FAC=8 fractional bits -> only
      // |diff| <~ 128 is representable in the int16 conversion below).
      // Real attention logit differences never approach this floor; only
      // entries carrying an extreme additive mask (e.g. bf16 min, used so
      // a fully-masked row round-trips to a single identical value and
      // therefore softmaxes to a uniform row) do. Without this clamp such
      // entries overflow/wrap the int16 conversion instead of cleanly
      // landing in the LUT's near-zero tail, which corrupts the whole
      // row's exp-sum. exp(kDiffFloor) is already ~0 in float32, so
      // clamping here is a no-op for every legitimate (non-masked) score.
      input_bf16 =
          aie::max(input_bf16,
                   aie::broadcast<bfloat16, VecLen>((bfloat16)kExpDiffFloor));
...
// Masked variant used for non-causal (prefix/padding-mask) attention: adds
// an additive mask (0 = attend, large-negative = masked out) to the row
// BEFORE the softmax. Numerically identical to softmax_bf16 once the mask
// has been folded in; the mask-add pass writes its result into the `out`
// scratch buffer, which is then treated as the softmax input (matching the
// in-place read-before-write block pattern already used by softmax_bf16's
// normalization loop).
void masked_softmax_bf16(const bfloat16 *__restrict in, const int pos,
                         const bfloat16 *__restrict mask,
                         bfloat16 *__restrict out) {
  int num_elems = pos + 1;
  const bfloat16 *__restrict pIn = in;
  const bfloat16 *__restrict pMask = mask;
  bfloat16 *__restrict pSum = out;

  // Step 1: out = in + mask
  for (unsigned i = 0; i < num_elems / 16 + (num_elems % 16 != 0); i++)
    chess_prepare_for_pipelining chess_loop_range(4, ) {
      aie::vector<bfloat16, 16> a_vec = aie::load_v<16>(pIn);
      aie::vector<bfloat16, 16> m_vec = aie::load_v<16>(pMask);
      aie::vector<bfloat16, 16> sum_vec = aie::add(a_vec, m_vec);
      aie::store_v(pSum, sum_vec);
      pIn += 16;
      pMask += 16;
      pSum += 16;
    }

  // Step 2: row-softmax of (in + mask), in place on `out`.
  const bfloat16 *__restrict pMaskedIn = out;
  bfloat16 *__restrict pExp = out;
  bfloat16 *__restrict pOut = out;
  float accum_exp_val = exp_bf16<16>(num_elems, pMaskedIn, pExp);
  bfloat16 accum_inv = compute_inv_as_bf16(accum_exp_val);
  for (unsigned i = 0; i < num_elems / 16 + (num_elems % 16 != 0); i++)
    chess_prepare_for_pipelining chess_loop_range(4, ) {
      aie::vector<bfloat16, 16> in_elems = aie::load_v<16>(pOut);
