...
"""Qwen2.5-0.5B end-to-end NPU inference (NPU prefill + NPU decode).

Pipeline (mirrors qwen25_1_5b/qwen25_inference.py with 0.5B-specific
shape config — head_dim=64 means we use standard seq-first FA, NOT the
Option C head-first wrapper).

    embed (CPU)
    --- prefill (per prompt) — PADDED shapes (emb=1024, hidden=5120) ---
    -> 24x [rms_gemms_rope (NPU 6-launch, bias added on host) ->
            {NPU FA seq-first | CPU attn} -> o_ffn (NPU 8-launch ELF)]
    ...
    -> 24x [rms_gemv_rope (NPU 6-launch, bias added on host
            via set_decode_position(pos)) ->
            CPU attention (KV cache) -> o_gemv_ffn (NPU 8-launch ELF,
            mv_k4864.o for Down GEMV)]
    -> CPU final RMSNorm + NPU LM Head GEMV -> argmax

Why split shapes between prefill (padded) and decode (orig):
    Prefill at seq_len=2048 needs padding because emb=896 / hidden=4864
    aren't 1024-aligned (would break o_ffn O GEMM tile_k_l2 divisibility
    and hit BD-pool exhaustion). Done via `qwen25_pad.pad_weights` with
    GQA-aware reindexing. Decode at M=1 doesn't have that blowup so we
    keep orig shapes for simplicity. The KV cache layout (n_kv_heads=2 ×
    max_seq × head_dim=64) is invariant under emb_dim padding.
"""
...
from qwen25_0_5b_phase3_test import _register_all_layer_biases
from qwen25_0_5b_phase5_test import _register_decode_biases
from qwen25_0_5b_decode_setup import (
    compile_qwen25_0_5b_decode_kernels,
    qwen25_0_5b_npu_lm_head_gemv,
    preload_qwen25_0_5b_lm_head,
)


def main():
    parser = argparse.ArgumentParser(
        description="Qwen2.5-0.5B end-to-end NPU inference (prefill + decode)"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use CPU attention in prefill (default: NPU FA seq-first; hd=64)",
    )
    parser.add_argument(
        "--npu-attn",
        dest="cpu_attn",
        action="store_false",
        help="Use NPU FA seq-first (default; no Option C since hd=64).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Compare NPU prefill (padded) vs CPU F32 reference (orig): "
            "per-layer K/V cache cosine + max_err for all 24 layers, plus "
            "final logits cosine at pred_pos and top-1 match. Adds ~30s wall."
        ),
    )
    ...
    print("Qwen2.5-0.5B end-to-end NPU inference")
    print(
    ...
    print(
        f"  attn={'CPU' if args.cpu_attn else 'NPU FA seq-first (hd=64)'}, "
    ...
    if not all(k in decode_cache.artifacts for k in needed_decode):
        compile_qwen25_0_5b_decode_kernels(decode_cache, orig_config)
    ...
    preload_qwen25_0_5b_lm_head(decode_cache, orig_weights, orig_config)
    ...
    last_normed_bf16 = (
        qwen25_0_5b_reference.rms_norm(last_hidden, orig_weights.final_norm)
        .flatten()
        .astype(bfloat16)
    )
    first_logits = qwen25_0_5b_npu_lm_head_gemv(
    ...
    if args.verify:
        print(f"\n{'='*60}")
        print(
            f"Verification: NPU prefill (padded) vs CPU F32 reference (orig) "
            f"({orig_config.n_layers} layers)"
        )
        print(f"{'='*60}")
        from qwen25_0_5b_reference import transformer_block as cpu_block
        ...
        x_cpu_normed = qwen25_0_5b_reference.rms_norm(
        ...
        print(
            f"  Per-layer warnings: {n_layer_warns} (informational; BF16 K/V "
            f"drift across deep stacks expected — see W1 in TODO.md)"
        )
    ...
    npu_decoded_ids = [first_token]  # for --verify multi-token greedy match
    for token_idx in range(args.n_tokens - 1):
        ...
        x_normed_bf16 = (
            qwen25_0_5b_reference.rms_norm(x_f32, orig_weights.final_norm)
            .flatten()
            .astype(bfloat16)
        )
        next_logits = qwen25_0_5b_npu_lm_head_gemv(
    ...
    # Multi-token greedy match check (--verify only)
    if args.verify and len(npu_decoded_ids) > 1:
        ...
        for di, _ in enumerate(npu_decoded_ids):
            ...
            for layer_idx in range(orig_config.n_layers):
                x_cpu, _ = qwen25_0_5b_reference.transformer_block(
                    x_cpu, orig_weights.layers[layer_idx], rope_lut_f32, orig_config
                )
            cpu_normed = qwen25_0_5b_reference.rms_norm(x_cpu, orig_weights.final_norm)
