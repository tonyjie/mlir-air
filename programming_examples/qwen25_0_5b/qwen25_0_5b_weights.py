...
Loads Qwen2.5-0.5B (Qwen2ForCausalLM) weights from HuggingFace safetensors
into numpy arrays usable by MLIR-AIR kernels.

Differences from the LlamaForCausalLM loader (`llama3_weights.py`,
`smollm2_weights.py`) handled here, mirroring `qwen25_weights.py` for
Qwen2.5-1.5B with shape config adjusted for 0.5B:

  - **QKV bias** (q_proj.bias / k_proj.bias / v_proj.bias) — Qwen2-family.
    Loaded into `LayerWeights.bq / bk / bv`. o_proj has no bias.
  - 24 layers, **head_dim=64** (vs 1.5B's 128), GQA group=7 (14 Q / 2 KV).
  - emb_dim=896 (NOT 1024-aligned), hidden_dim=4864 (NOT 1024-aligned;
    K<8160 so no down_k_split needed).
  - vocab=151936, rope_base=1e6.
  - tie_word_embeddings=True → same fallback path as 1.5B and Llama-3.2-1B.
...
@dataclass
class LlamaConfig:
    """Qwen2.5-0.5B model hyperparameters (Qwen2ForCausalLM architecture).
...
class LayerWeights:
    ...
    Attributes:
        ...
        wq:         (emb_dim, n_heads*head_dim)   = (896, 896)
        wk:         (emb_dim, n_kv_heads*head_dim)= (896, 128)
        wv:         (emb_dim, n_kv_heads*head_dim)= (896, 128)
        bq:         (n_heads*head_dim,)           = (896,)
        bk:         (n_kv_heads*head_dim,)        = (128,)
        bv:         (n_kv_heads*head_dim,)        = (128,)
        wo:         (emb_dim, emb_dim)            = (896, 896)
        ffn_norm:   (emb_dim,)
        w_gate:     (emb_dim, hidden_dim)         = (896, 4864)
        w_up:       (emb_dim, hidden_dim)         = (896, 4864)
        w_down:     (hidden_dim, emb_dim)         = (4864, 896)
    """
...
def load_weights(
    ...
    """Load Qwen2.5-0.5B weights from safetensors into numpy arrays."""
...
def generate_rope_lut(
    ...
    For Qwen2.5-0.5B, theta = 1,000,000 (set in config.rope_base).
    """
    if config is None:
        config = LlamaConfig()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Load Qwen2.5-0.5B weights and print shapes",
    )
    parser.add_argument(
        "model_path",
        type=str,
        ...
        help="Local dir or HF model ID (default: Qwen/Qwen2.5-0.5B)",
    )
    parser.add_argument(
        "--dtype",
        type=str,
