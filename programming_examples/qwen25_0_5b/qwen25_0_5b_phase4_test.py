...
"""Phase 4 — prefill performance for Qwen2.5-0.5B on NPU2.

NPU forward uses padded shapes (emb=1024, hidden=5120, n_heads=16) plus
GQA-reindexed weights, Qwen2 host bias add, and standard NPU seq-first
FA (head_dim=64 → no Option C wrapper).
...
def main():
    parser = argparse.ArgumentParser(description="Qwen2.5-0.5B Phase 4 prefill perf")
    ...
    parser.add_argument("--n-warm-runs", type=int, default=5)
    ...
    print(
        f"Qwen2.5-0.5B padded config: n_layers={padded_config.n_layers}, "
    ...
    print(f"Attention: {'CPU' if args.cpu_attn else 'NPU FA seq-first (hd=64)'}")
    ...
    print()
    print("=" * 60)
    print(f"PATTERN B1 — preload_prefill_weights ({padded_config.n_layers} layers)")
    print("=" * 60)
    t = time.time()
    preload_prefill_weights(
    ...
    warm_layer_std = float(np.std(warm_layer_times))
    ...
    print(
        f"  Warm avg NPU layer time          : {warm_layer_avg:.3f} s ± {warm_layer_std*1000:.1f} ms"
    )
    if npu_layer_t_cold > 0:
        gain = npu_layer_t_cold - warm_layer_avg
        pct = (1 - warm_layer_avg / npu_layer_t_cold) * 100
        print(
            f"  Pattern B1 gain on first prompt  : {gain:.3f} s ({pct:.1f}% reduction)"
        )
    ...
    print(
        "  [INHERITED] A. Multi-launch merging      (rms_gemms_rope=6, o_ffn=8 launches; "
        "from llama3/multi_launch_builder/)"
    )
    print(
        f"  [APPLIED  ] B1. Per-layer BO pre-loading  (preload_prefill_weights, {preload_t:.2f} s setup)"
    )
    print(
        "  [INHERITED] B2. Intermediate buffer reuse (intermediate_indices set per kernel)"
    )
    print(
        "  [INHERITED] C. Seq-first layout          (RoPE/FA native; no host transposes)"
    )
    if args.cpu_attn:
        print(
            "  [SKIPPED  ] D. CPU→NPU op promotion     "
            "(--cpu-attn explicitly set; FA on CPU)"
        )
    else:
        print(
            "  [APPLIED  ] D. CPU→NPU op promotion     "
