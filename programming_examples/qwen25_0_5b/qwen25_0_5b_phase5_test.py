...
"""Phase 5 — decode performance for Qwen2.5-0.5B on NPU2.

CPU prefill seeds the KV cache (Phase 5 design, llama3 pattern), then NPU
decode loop with KV cache. Decode uses ORIG shapes (emb=896, hidden=4864,
n_heads=14) — at M=1 the BD-pool exhaustion that forced prefill padding
doesn't apply.

Pattern application (5/5):
  A.  Multi-launch merging      INHERITED (rms_gemv_rope=6, o_gemv_ffn=8)
  B.  Static weight BOs         INHERITED (preload_decode_weights via shared infra)
  D1. NPU LM Head GEMV          APPLIED (10×16384 partitions, vocab=151936)
  D2. Extern kernel rename      APPLIED (mv_k4864.o for Down GEMV K=4864)
  D3. CPU->NPU op promotion     PARTIAL (attention stays CPU per llama3 design)
"""
...
def main():
    parser = argparse.ArgumentParser(description="Qwen2.5-0.5B Phase 5 decode perf")
    ...
    config = LlamaConfig()
    print(
        f"Qwen2.5-0.5B (orig shapes for decode): n_layers={config.n_layers}, "
    ...
    if not all(k in cache.artifacts for k in needed):
        print("\nCompiling decode kernels for Qwen2.5-0.5B shapes...")
        t = time.time()
        compile_qwen25_0_5b_decode_kernels(cache, config)
    ...
    print(
        f"Pre-loading LM head BOs (10 partitions × 16384, vocab={config.vocab_size})..."
    )
    t = time.time()
    ...
    for token_idx in range(args.n_tokens - 1):
        ...
        x_normed_bf16 = (
            qwen25_0_5b_reference.rms_norm(x_f32, weights.final_norm)
            .flatten()
            .astype(bfloat16)
        )
        npu_logits = qwen25_0_5b_npu_lm_head_gemv(cache, weights, config, x_normed_bf16)
        ...
        if args.cpu_verify:
            ...
            for layer_idx in range(config.n_layers):
                cpu_x, _ = qwen25_0_5b_reference.transformer_block(
                    cpu_x, weights.layers[layer_idx], rope_lut_f32, config
                )
            cpu_normed = qwen25_0_5b_reference.rms_norm(cpu_x, weights.final_norm)
    ...
    print(
        "    [INHERITED] A. Multi-launch merging      (rms_gemv_rope=6, o_gemv_ffn=8)"
    )
    print("    [INHERITED] B. Static weight BOs         (decode preload)")
    print(
        f"    [APPLIED  ] D1. NPU LM Head GEMV         (10×16384 partitions, vocab={config.vocab_size})"
    )
    print(
        "    [APPLIED  ] D2. Extern kernel rename     (mv_k4864.o for Down GEMV K=4864)"
    )
    print(
        "    [PARTIAL  ] D3. CPU->NPU op promotion    (attention stays CPU per llama3 design)"
    )
