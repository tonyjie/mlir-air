...
int main(int argc, const char *argv[]) {
  ...
  options.add_options()("help,h", "produce help message")(
      ...
      "num-heads", "Number of Q attention heads",
      cxxopts::value<int>()->default_value("32"))(
      "num-kv-heads", "Number of KV attention heads",
  ...
  // SEQ-FIRST layout: [seq, heads * dim]
  size_t Q_VOLUME = (size_t)lq * num_heads * dk;      // [lq, num_heads*dk]
  size_t K_VOLUME = (size_t)lk * num_kv_heads * dk;   // [lk, num_kv_heads*dk]
  size_t V_VOLUME = (size_t)lk * num_kv_heads * dv;   // [lk, num_kv_heads*dv]
  size_t OUTPUT_VOLUME = (size_t)lq * num_heads * dv; // [lq, num_heads*dv]
  ...
  std::cout << "Flash Attention Benchmark — SEQ-FIRST (ELF format)"
            << std::endl;
  std::cout << "  num_heads=" << num_heads << ", num_kv_heads=" << num_kv_heads
            << ", lq=" << lq << ", lk=" << lk << ", dk=" << dk << ", dv=" << dv
            << std::endl;
  std::cout << "  Q: [" << lq << "x" << num_heads * dk << "] (" << Q_SIZE
            << " bytes)" << std::endl;
  std::cout << "  K: [" << lk << "x" << num_kv_heads * dk << "] (" << K_SIZE
            << " bytes)" << std::endl;
  std::cout << "  V: [" << lk << "x" << num_kv_heads * dv << "] (" << V_SIZE
            << " bytes)" << std::endl;
  std::cout << "  Output: [" << lq << "x" << num_heads * dv << "] ("
