...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Compare NPU prefill (padded shapes) vs CPU F32 reference (orig "
            "shapes): per-layer K/V cache cosine + max_err + mean_err for all "
            "28 layers, plus final logits cosine at pred_pos and top-1 match. "
            "Mirrors llama3 / llama32_3b --verify; KV cache layout "
            "(n_kv_heads, seq, head_dim) is invariant under emb_dim padding "
            "so NPU and CPU caches are directly comparable. Adds ~30s wall."
        ),
    )
    ...
    if args.verify:
        ...
        print(f"{'='*60}")
        from qwen25_reference import transformer_block as cpu_block
        ...
        for li in range(orig_config.n_layers):
            ...
            # KV cache layout (n_kv_heads, max_seq, head_dim) is invariant
            # under emb_dim padding because n_kv_heads stays at 2. So padded
            # prefill's NPU k_cache slot is directly comparable to orig CPU.
        ...
        x_cpu_normed = qwen25_reference.rms_norm(
