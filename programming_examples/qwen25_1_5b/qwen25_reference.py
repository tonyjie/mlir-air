...
"""CPU reference forward pass for Qwen2.5-1.5B (Qwen2ForCausalLM).

Pure NumPy in F32 for high-accuracy verification of NPU outputs.

Differences from the Llama reference (`llama3_reference.py`,
`smollm2_reference.py`):
  - Q/K/V projections add Qwen2 bias (`q + bq`, `k + bk`, `v + bv`).
  - rms_norm_eps = 1e-6 (Qwen2 default), pulled from config.
  - head_dim = 128 (vs 64 in prior models) — handled generically
    by the existing apply_rope path.

GQA reduction: n_heads=12, n_kv_heads=2 → group_size=6 per KV head.
"""

import argparse
import numpy as np

from qwen25_weights import (
    LlamaConfig,
    LayerWeights,
    LlamaWeights,
    load_weights,
...
def attention_reference(q, k, v, n_heads, n_kv_heads):
    ...
    For Qwen2.5-1.5B: 12 Q heads, 2 KV heads, group_size = 6.
    """
...
def forward(token_ids, weights, config, rope_lut=None):
    """Full Qwen2.5-1.5B forward pass."""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="CPU reference forward pass for Qwen2.5-1.5B"
    )
