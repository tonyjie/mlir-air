...
def run_gate(
    ...
    oracle = _HERE / "smolvla_oracle.npz"
    if not oracle.exists():
        raise FileNotFoundError(
            f"{oracle} not found — run `make oracle` first (CPU only, no NPU)."
        )
    ref = np.load(oracle)["action_chunk"]  # (1,50,6) pure-CPU, fixed zero noise
...
def main() -> int:
    ...
    --cpu-vision runs the unmodified model against its own baseline, which
    should score 1.0 exactly -- a useful sanity check of the harness itself.
    """
