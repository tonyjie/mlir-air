# LLAMA-3.2-1B — Performance Optimization History

## Overview

Performance optimization journey for LLAMA-3.2-1B BF16 on NPU2 (AIE2P), covering
both prefill (seq_len=2048) and decode (single token). Compared against
IRON (`/home/jiajli/apps/IRON`).

See `docs/profile.md` for the latest detailed breakdown.
...
|-------|------------|------|---------|
| **Prefill** | **1.30s kernel / 1.54s wall** | 2.744s | **2.1x (kernel)** |
| **Decode** | **92ms/token** | 370ms/token | **4.0x** |
| **Throughput** | **10.8 tok/s** | 2.7 tok/s | **4.0x** |

## Prefill Optimization Journey

| Step | Time | vs IRON | Key Change |
|------|------|---------|------------|
| Initial (CPU reference) | 18.67s | 6.8x slower | All on CPU |
| + BF16 eltwise add | 13.40s | 4.9x slower | NPU eltwise ops |
| + BO reuse | 6.49s | 2.4x slower | XRT context caching |
| + Optimized GEMM | 3.60s | 1.3x slower | 8×4 herd, optimal tiles |
| + NPU FlashAttention | 2.45s | 11% faster | Replaced CPU fallback |
| + Multi-launch merges | 1.77s | 35% faster | 10→5 invocations/layer |
| + Seq-first + more merges | 1.49s | 46% faster | 5→3 invocations, zero transposes |
| + Weight pre-loading | 1.25s (kernel) | 2.2x faster | Per-layer BOs, intermediate_indices |
| + Host overhead cleanup | **1.30s kernel / 1.54s wall** | **2.1x (kernel)** | Remove dead code, single-row LM Head, verbose guards |

## Decode Optimization Journey

| Step | Time | vs IRON | Key Change |
|------|------|---------|------------|
| Initial (10 XRT calls) | ~500ms | 1.4x slower | CPU LM Head (258ms) |
| + Static weight BOs | 407ms | ~same | Weight reuse across tokens |
| + Multi-launch merge (2 calls) | 365ms | ~same | rms_gemv_rope + o_gemv_ffn |
| + NPU LM Head GEMV | **92ms** | **4.0x faster** | 8-partition GEMV (13.5ms) |

## Historical Prefill Comparison (detailed)
