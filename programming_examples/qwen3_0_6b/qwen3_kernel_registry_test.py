...
Kernels exercised (Qwen3-0.6B decode shapes, M=1):
...
def main():
    parser = argparse.ArgumentParser(
        description="Qwen3-0.6B Step 1 — standalone kernel sweep"
    )
    parser.add_argument(
        "--filter",
        type=str,
    ...
    # Define the suite (label, callable). Single-arg callables for simplicity.
    suite = [
        ("rms_norm_M1_N1024", lambda: test_weighted_rms_norm(M=1, N=1024)),
        ("gemv_Q_M2048_K1024", lambda: test_gemv(m=2048, k=1024, label_suffix="_Q")),
        ("gemv_KV_M1024_K1024", lambda: test_gemv(m=1024, k=1024, label_suffix="_KV")),
        ("gemv_O_M1024_K2048", lambda: test_gemv(m=1024, k=2048, label_suffix="_O")),
        (
            "gemv_GateUp_M3072_K1024",
            lambda: test_gemv(m=3072, k=1024, label_suffix="_GU"),
        ),
        ("gemv_Down_M1024_K3072", lambda: test_gemv(m=1024, k=3072, label_suffix="_D")),
        (
            "gemv_LMhead_M16384_K1024",
            # qwen25_decode_setup uses tile_m=16, m_input=16, herd_m=8 for the
            # LM head 16384 partition: M/(tile_m*herd_m) = 16384/128 = 128 outer
            # iters (the default 8/4/8 → 256 iters → AIE2P shim repeat>255 hard fail).
            lambda: test_gemv(
                m=16384,
                k=1024,
                label_suffix="_LM",
        ...
        ("silu_and_mul_n3072", lambda: test_silu_and_mul(n=3072)),
        # Phase A new leaves
    ...
    print(f"\n{'='*72}")
    print(
        f"Qwen3-0.6B Step 1 — standalone kernel registry sweep ({len(suite)} kernels)"
    )
    print(f"{'='*72}\n")
