...
- `programming_examples/_llm_shared/docs/compiler_issues/herd_load_bug.md`
  — `airrt.herd_load` vs `airrt.segment_load` semantics
- `programming_examples/_llm_shared/kernel_builder/stitching.py`
  — `_wrap_ir_in_launch` is the helper that wraps a bare herd in
  `air.launch + air.segment` (see hypothesis 4 below)
...
"succeeds" silently but never actually executes the DMA path. This was
the qwen25_1_5b silent-corruption pattern (cosine ≈ 0 / NaN).
...
`_llm_shared/kernel_builder/stitching.py`. Reference implementations
that use this wrapper:

- `_llm_shared/kernel_builder/rmsnorm_multitile/run.py` — wraps
  `weighted_rms_norm.build_module(herd_x=8)` (which emits bare herd)
- `_llm_shared/kernel_builder/eltwise_add_multitile/run.py` — same
  pattern for Eltwise Add at `herd_x=8`
