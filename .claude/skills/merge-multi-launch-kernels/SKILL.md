...
- `programming_examples/llama3/multi_launch_builder/rms_gemms_rope_multi.py`
  — reference 6-launch merge (RMSNorm + Q/K/V GEMM + RoPE Q/K)
- `programming_examples/llama3/multi_launch_builder/o_ffn_multi.py`
  — reference 8-launch merge (O + add + RMSNorm + Gate/Up + SwiGLU + Down + add)
- `programming_examples/qwen3_0_6b/multi_launch/o_gemv_ffn_silu_qwen3.py`
  — kernel-first 3-K extern rename example (when `n_heads*head_dim != emb_dim`)
- `programming_examples/_llm_shared/docs/multi-launch/host_optimization.md`
  — host-side BO write/read overhead, why merging helps
...
2. Imports stitching helpers: `from _llm_shared.kernel_builder.stitching import (_rename_all, _fix_launch_func_args, _wrap_ir_in_launch, ...)`
...
7. For multi-K GEMV in one ELF (decode kernel-first): use
   ...
   different `mv_*.o` symbols distinct (qwen3-0.6B 3-K example)
