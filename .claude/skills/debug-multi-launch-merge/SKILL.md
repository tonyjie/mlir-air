...
- `programming_examples/_llm_shared/docs/compiler_issues/multi_launch_blockers.md`
  — exhaustive list of merge constraints
- `programming_examples/_llm_shared/docs/compiler_issues/multi_launch_root_cause.md`
  — root cause analyses
- `programming_examples/_llm_shared/docs/multi-launch/full_block.md`
  — what does NOT merge (FlashAttention)
- `programming_examples/_llm_shared/docs/multi-launch/compiler_scaling.md`
  — compile time scaling with ELF size
- `programming_examples/_llm_shared/docs/compiler_issues/weight_broadcast_dma.md`
  — DMA stride=1 limitation for sub-32b types (BF16 broadcast)
- `programming_examples/_llm_shared/docs/aie2p_hardware_limits.md`
  — Rule A (BD inner-dim 1024), Rule B (GEMV K-DMA repeat), etc.
...
buffer descriptors`. Often triggered by Rule A (non-1024-aligned dim)
because the DMA auto-splits into many shim BDs that exhaust the pool.

**Diagnostic**: identify which model dim is non-1024-aligned (qwen25_1_5b
LESSON 4 was `emb_dim=1536`). Check the merged ELF's launch count and
whether each launch's DMA pattern is BD-friendly (Rule A in
`aie2p_hardware_limits.md`).

**Remedy** (in order of preference):

1. Pad the offending dim to 1024-aligned via GQA-aware reindexed
   padding (`programming_examples/qwen25_1_5b/qwen25_pad.py`)
...
`_llm_shared/kernel_builder/stitching.py` already prefixes every SSA
...
`_llm_shared/kernel_builder/stitching.py`. Reference uses:
`_llm_shared/kernel_builder/{rmsnorm_multitile, eltwise_add_multitile}/run.py`.
