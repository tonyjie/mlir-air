...
diagnosis efficiently instead of bisecting from scratch — past
deployments (llama32_3b, qwen25_1_5b) hit each of these, all are
...
- `programming_examples/llama32_3b/docs/development_progress/LESSONS.md`
  — Lesson 3 full root-cause writeup (the original llama32_3b debug
  session that codified these 3 causes)
...
- `programming_examples/_llm_shared/kernel_builder/external_kernels.py`
  — `compile_attn_npu2_split(lqp, lkp, dk, dv, num_q_tiles=4)` — the
  fixed compile API that derives per-tile flags correctly
- `programming_examples/_llm_shared/phase_helpers/headfirst_fa.py`
  — `install_headfirst_fa_wrapper()` + `compile_headfirst_fa_kernel(...)` —
  the Option C reusable wrapper for head_dim=128
...
**Diagnostic**: diff your compile call against the Makefile. The fixed
`compile_attn_npu2_split(lqp, lkp, dk, dv, num_q_tiles=4)` in
`_llm_shared/kernel_builder/external_kernels.py` derives `lqp_tile =
lqp // num_q_tiles` internally and emits the right flags.

**Remedy**: if your `.o` was compiled with per-launch flags (or
predates the `compile_attn_npu2_split` fix), delete the stale `.o`
and the cached `flash_attn.elf`, then rebuild via
`compile_attn_npu2_split`. Re-run.
...
`attn_npu2.py` at the same shape PASSES (lit-tested by llama3-8b at
`make run DK=128 DV=128 NUM_HEADS=32 NUM_KV_HEADS=8` → corr = 0.996).

**Remedy** — Option C (head-first FA + host transposes), proven on
llama32_3b:

1. In `compile_block_kernels`, use the **reusable Option C wrapper**
   from `_llm_shared/phase_helpers/headfirst_fa.py`:
   ```python
   from _llm_shared.phase_helpers.headfirst_fa import (
       install_headfirst_fa_wrapper, compile_headfirst_fa_kernel,
   )
   install_headfirst_fa_wrapper()  # idempotent, monkey-patches once
   compile_headfirst_fa_kernel(cache, seq_len, n_heads, n_kv_heads, head_dim)
   ```
2. The wrapper handles all the I/O transposes automatically. It uses
   `attn_npu2.build_module(...)` (head-first) under the hood and
   intercepts `_run_cached("flash_attn", ...)` to do
   seq-first ↔ head-first reshapes around the call.

Cost: a few ms/layer host transpose. Gain: NPU FA actually runs
(llama32_3b: 13.6 s CPU-attn → 3.2 s NPU FA = 4.2× warm prefill
speedup).
...
The pattern is straightforward — invoke `compile_attn_npu2_split(...)`
to compile the .o at a given shape, build the module via
`attn_npu2[_seqfirst].build_module(...)`, generate random inputs +
NumPy causal-SDPA reference, run via XRTRunner. Catch `TIMEOUT` →
"HANG"; cosine < threshold → "FAIL_NUMERICAL". For a worked example
see `programming_examples/llama32_3b/llama32_3b_phase{2,3,4}_test.py`
which exercises FA at the production shape.
